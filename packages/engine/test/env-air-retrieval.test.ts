...
describe("environment air retrieval", () => {
  ...
  it("prioritizes fixed-source CEMS applicability over ambient automatic station evidence", async () => {
    const { wikiDir, dbPath } = await createTempWorkspace();
    const pages = [
      {
        page: graphPage("source:ambient-hj655", "HJ 655-2013 环境空气颗粒物连续自动监测系统安装和验收技术规范", "sources/ambient-hj655.md"),
        data: {
          authority_layer: "method",
          legal_status: "current_effective",
          document_role: "monitoring_method",
        ...
        body: "# HJ 655-2013\n\n环境空气自动站安装验收材料也会出现采样入口、联网、数据传输和现场验收等词，但适用对象是环境空气自动监测站。"
      },
      {
        page: graphPage(
          "source:ambient-hj818",
          "HJ 818-2018 环境空气气态污染物连续自动监测系统运行和质控技术规范",
          "sources/ambient-hj818.md"
        ),
        data: { authority_layer: "method", legal_status: "current_effective", document_role: "qa_qc", standard_code: "HJ 818-2018" },
        body: "# HJ 818-2018\n\n环境空气气态污染物自动监测系统运行质控，涉及数据有效性、校准、零点漂移和运行检查。"
      },
      {
        page: graphPage("source:hj75", "HJ 75-2017 固定污染源烟气排放连续监测技术规范", "sources/hj75.md"),
        data: {
        ...
        body: "# HJ 75-2017\n\n固定污染源烟气CEMS采样探头、采样系统、采样探头位置变化核查、运行维护、质控检查和固定源连续监测技术规范。"
      },
      {
        page: graphPage("source:hj76", "HJ 76-2017 固定污染源烟气排放连续监测系统技术要求及检测方法", "sources/hj76.md"),
        data: {
          authority_layer: "method",
          legal_status: "current_effective",
          document_role: "monitoring_method",
          standard_code: "HJ 76-2017"
        },
        body: "# HJ 76-2017\n\n固定污染源烟气CEMS系统技术要求、检测方法、性能指标和设备验收相关要求。"
      },
      {
        page: graphPage("source:hj212", "HJ 212-2025 污染物自动监测监控系统数据传输标准", "sources/hj212.md"),
        data: { authority_layer: "method", legal_status: "current_effective", document_role: "standard", standard_code: "HJ 212-2025" },
        body: "# HJ 212-2025\n\n污染物在线监控、固定污染源CEMS和数采仪的数据传输协议、通信联网和数据上传要求。"
      },
      {
        page: graphPage("source:order19", "污染源自动监控设施现场监督检查办法 环境保护部令第19号", "sources/order19.md"),
        data: {
          authority_layer: "core",
          legal_status: "current_effective",
          document_role: "regulation",
        ...
        body: "# 19号令\n\n污染源自动监控设施现场监督检查办法规定现场检查、采样探头位置变化、数据异常、事实证据、程序证据和监管处理边界。"
      },
      {
        page: graphPage("source:auto-monitoring-28", "污染源自动监控管理办法 国家环保总局令第28号", "sources/auto-monitoring-28.md"),
        data: {
          authority_layer: "core",
          legal_status: "current_effective",
          document_role: "regulation",
        ...
        body: "# 28号令\n\n污染源自动监控管理办法规定排污单位、运维单位、自动监控设施运行维护、停运拆除、故障报告、设备更换和联网管理职责。"
      },
      {
        page: graphPage("source:daily-penalty-28", "环境保护主管部门实施按日连续处罚办法 第28号令", "sources/daily-penalty-28.md"),
        data: { authority_layer: "core", legal_status: "current_effective", document_role: "regulation", standard_code: "第28号令" },
        body: "# 按日连续处罚办法\n\n本办法规定按日连续处罚程序，不是污染源自动监控管理办法。"
      }
    ];
    for (const item of pages) {
      await writePage(wikiDir, item.page.path, item.data, item.body);
    ...
    const inspectionResults = searchPages(dbPath, "固定源现场检查里，采样探头位置变化和数据传输异常分别应引用哪类依据？", {
      limit: 6,
      authorityLayer: ["core", "method"],
      includeDrafts: false,
      includeSuperseded: false
    }).map((result) => result.pageId);
    const uniqueInspectionResults = [...new Set(inspectionResults)];
    expect(uniqueInspectionResults.slice(0, 4)).toContain("source:order19");
    expect(
      uniqueInspectionResults.slice(0, 4).some((pageId) => pageId === "source:hj75" || pageId === "source:hj76"),
      uniqueInspectionResults.join(",")
    ).toBe(true);
    expect(uniqueInspectionResults.slice(0, 4)).toContain("source:hj212");
    expect(uniqueInspectionResults.slice(0, 3)).not.toContain("source:ambient-hj655");

    const managementResults = searchPages(dbPath, "自动监控管理里说的28号令，是不是按日连续处罚那个28号令？引用时怎么避免混？", {
      limit: 5,
      authorityLayer: ["core", "method"],
      includeDrafts: false,
      includeSuperseded: false
    }).map((result) => result.pageId);
    expect(managementResults.slice(0, 3)).toContain("source:order19");
    expect(managementResults.slice(0, 3)).toContain("source:auto-monitoring-28");
    expect(managementResults.indexOf("source:auto-monitoring-28")).toBeLessThan(managementResults.indexOf("source:daily-penalty-28"));
  });
